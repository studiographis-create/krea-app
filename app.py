...
CATEGORIES = ["Photoshop", "Lightroom", "InDesign", "Illustrator", "AI", "Graphisme", "Photo", "Tutoriels", "Expos photos", "☆ Favoris"]
...
# --- BARRE DE FILTRES MULTI-CATÉGORIES SUR UNE LIGNE (PASTILLES) ---
st.write("Filtres par catégories :")
cat_cols = st.columns(len(CATEGORIES))
for idx, cat in enumerate(CATEGORIES):
    with cat_cols[idx]:
        is_active = cat in st.session_state.selected_cats
        btn_label = f"✓ {cat}" if is_active else cat
        if st.button(btn_label, key=f"cat_btn_{idx}", use_container_width=True):
            if is_active:
...
for art in all_fetched:
    ...
    # Logique de filtrage multi-catégories par pastilles
    if not st.session_state.selected_cats:
        cat_match = True
    else:
        cat_match = False
        for sel_cat in st.session_state.selected_cats:
            if sel_cat == "☆ Favoris":
                if art["link"] in st.session_state.bookmarks:
                    cat_match = True
                    break
            elif sel_cat == "Expos photos":
                ...
                if is_expos_source or kw_match:
                    cat_match = True
                    break
            else:
                kw_list = KEYWORDS.get(sel_cat, [])
                if any(re.search(r'\b' + re.escape(kw) + r'\b', text_to_check) for kw in kw_list) or art["category"] == sel_cat:
                    cat_match = True
                    break
...
if not st.session_state.selected_cats and selected_source == "Toutes les sources" and not search_query.strip() and st.session_state.category_views:
    filtered_articles.sort(key=lambda x: (st.session_state.category_views.get(x["category"], 0), x["date"]), reverse=True)

if st.session_state.selected_cats == ["☆ Favoris"] and filtered_articles:
    ...
    with col_fav_json:
        st.download_button("⤓ JSON", data=json.dumps(filtered_articles, indent=2, ensure_ascii=False), file_name="favoris_krea.json", mime="application/json", use_container_width=True)
    with col_fav_md:
...
if filtered_articles:
    show_hero = (not st.session_state.selected_cats and selected_source == "Toutes les sources" and not search_query.strip() and view_mode == "Grille")
    start_idx = 0
