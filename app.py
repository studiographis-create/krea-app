    ...
    for feed in SOURCES:
        try:
            ...
            if resp.status_code == 200:
                ...
                # LIMITÉ À 4 ARTICLES PAR FLUX POUR ÉVITER LES SURCHARGES DE MÉMOIRE (RAM)
...
if filtered_articles:
    visible_articles = filtered_articles[:st.session_state.articles_limit]
    ...
    if view_mode == "Grille":
        ...
        for idx, article in enumerate(visible_articles):
            ...
            with col:
                with st.container(border=True):
                    ...
                    st.markdown(f'<img src="{article["image_url"]}" style="width:100%; height:160px; object-fit:cover; border-radius:10px; margin-bottom:8px; display:block;">', unsafe_allow_html=True)
                    st.markdown(f'<span class="cat-badge" style="background-color:{cat_color};">{article["category"]}</span>', unsafe_allow_html=True)
                    if is_read: st.markdown('<span class="read-badge">✓ Lu</span>', unsafe_allow_html=True)
        ...
        for article in visible_articles:
            ...
            with st.container(border=True):
                ...
                with c_img:
                    st.markdown(f'<img src="{article["image_url"]}" style="width:100%; height:100px; object-fit:cover; border-radius:8px; display:block;">', unsafe_allow_html=True)
                with c_content:
                    st.markdown(f'<span class="cat-badge" style="background-color:{cat_color};">{article["category"]}</span>', unsafe_allow_html=True)
                    if is_read: st.markdown('<span class="read-badge">✓ Lu</span>', unsafe_allow_html=True)
                    st.caption(f"⌖ **{article['source']}** • {article['relative_date']}")
