...
# --- CONFIGURATION LOGO ---
KREA_SVG_ICON = """<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><defs><linearGradient id="g1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#8B5CF6"/><stop offset="100%" stop-color="#2563EB"/></linearGradient></defs><rect x="0" y="0" width="100" height="100" rx="22" fill="#0b0f19"/><rect x="12" y="12" width="62" height="62" rx="14" fill="url(#g1)"/><text x="43" y="56" font-family="sans-serif" font-weight="900" font-size="44" fill="#FFFFFF" text-anchor="middle" transform="rotate(-10 43 56)">k</text></svg>"""
...
# --- STYLE CSS (Inclus la transformation des boutons en petites pastilles) ---
st.markdown("""
<style>
    header {visibility: hidden;}
    .stApp {background-color: #0b0f19; color: #f1f5f9;}
    /* Transformation des boutons en petites pastilles compactes */
...
# Filtres catégories en ligne (Pastilles)
st.write("Catégories :")
...
for i, cat in enumerate(CATEGORY_COLORS.keys()):
    with cat_cols[i]:
        ...
        if st.button(f"{'● ' if is_active else '○ '}{cat}", key=f"filter_{cat}"):
