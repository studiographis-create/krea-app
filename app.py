...
categories_list = ["Photoshop", "Lightroom", "InDesign", "Illustrator", "AI", "Graphisme", "Photo", "Tutoriels", "Expos photos", "Général", "☆ Favoris"]
...
for art in all_fetched:
    ...
    # Logique de filtrage multi-catégories
    if not selected_categories:
        cat_match = True
    else:
        cat_match = False
        if "☆ Favoris" in selected_categories and art["link"] in st.session_state.bookmarks:
            cat_match = True
            
        other_cats = [c for c in selected_categories if c != "☆ Favoris"]
        if other_cats:
            for cat in other_cats:
                if cat == "Expos photos":
                    ...
                    kw_list = KEYWORDS.get(cat, [])
                    if any(re.search(r'\b' + re.escape(kw) + r'\b', text_to_check) for kw in kw_list):
                        cat_match = True
                        break
                        
        if "☆ Favoris" in selected_categories and len(selected_categories) == 1:
            cat_match = (art["link"] in st.session_state.bookmarks)
...
if not selected_categories and selected_source == "Toutes les sources" and not search_query.strip() and st.session_state.category_views:
    filtered_articles.sort(key=lambda x: (st.session_state.category_views.get(x["category"], 0), x["date"]), reverse=True)

if selected_categories == ["☆ Favoris"] and filtered_articles:
...
if filtered_articles:
    show_hero = (not selected_categories and selected_source == "Toutes les sources" and not search_query.strip() and view_mode == "Grille")
    start_idx = 0
