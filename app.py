...
if selected_category == "⭐ Favoris" and filtered_articles:
    ...
    with col_fav_title: st.subheader(f"📌 Vos articles favoris ({len(filtered_articles)})")
    with col_fav_exp:
        json_favs = json.dumps(filtered_articles, indent=2, ensure_ascii=False)
        st.download_button("📥 Exporter les favoris (JSON)", data=json_favs, file_name="favoris_krea.json", mime="application/json", use_container_width=True)
    ...
    if show_hero and len(filtered_articles) > 0:
        ...
        with st.container(border=True):
            ...
            with col_hero_text:
                ...
                with c1: st.link_button("Lire l'article", hero['link'], use_container_width=True)
                with c2:
...
<div style="text-align: center; margin-top: 60px; padding: 30px 0 10px 0; border-top: 1px solid rgba(255, 255, 255, 0.08);">
    <div style="display: flex; justify-content: center; align-items: center; margin-bottom: 8px;">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 110 110" width="50" height="50" style="filter: drop-shadow(0px 4px 12px rgba(139, 92, 246, 0.3));">
          ...
          <text x="53" y="64" font-family="sans-serif" font-weight="900" font-size="45" fill="#FFFFFF" text-anchor="middle" transform="rotate(-10 53 64)">k</text>
        </svg>
    </div>
    <p style="color: #94A3B8; font-size: 0.85rem; font-weight: 600; letter-spacing: 0.5px; margin: 0;">by Graphis Studio</p>
</div>
""", unsafe_allow_html=True)        font-size: 0.95rem !important;
        opacity: 1 !important;
    }
